...
(function(){
    ...
    function ListController(quizMetrics, DataService){
         ...
         */
		 ...
		 // pour le son
		vm.playSongStart=playSongStart;
        vm.quizMetrics = quizMetrics; // Controllers reference to the quiz data from factory
        vm.testOf = testOf; // Controllers reference to the quiz data from factory
        vm.data = DataService.turtlesData; // Controller reference to the turtle info created in the factory
        vm.config = DataService.configArray.quiz_title; // Controller reference to the turtle info created in the factory
		...
		vm.configTitle = DataService.configArray.quiz_title; 
		vm.configTitle2 = DataService.configArray.quiz_title2; 
		vm.configSubLines = DataService.configArray.SubLines; 
		
		function testOf(){
			audio.play();
		}
		 function playSongStart(){	addTenLimit();
			
			var newUrl=$('#questionPerf').attr("pk-src");
			$('#questionPerf').attr("src",newUrl);
	...
	// angular.element(document.getElementById('juro')).scope().roundProgressData.label=-0;			
